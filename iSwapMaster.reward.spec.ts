...
const iSwapStartBlock = new BigNumber(4).mul(new BigNumber(10).pow(17)) // 一块0.4个
...
// 0.11
const iSwapBonusEndBlock: BigNumber = iSwapStartBlock
  .sub(
    bonusEndBlock
      .sub(startBlock)
...
console.log('iSwapBonusEndBlock  ' + iSwapBonusEndBlock)
...
const getTotalRewardInfoInSameCommonDifference = (
  ...
  console.log('_to ' + _to)
  console.log('_iSwapInitBlock ' + _iSwapInitBlock)
  ...
  console.log('last block create iSwap ' + _iSwapInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)))
  console.log('totalReward ' + totalReward)

  if (lastRewardBulkLastBlock.gt(_from) && lastRewardBulkLastBlock.lte(_to)) {
    let diff = lastRewardBulkLastBlock
      .sub(_from)
      .mul(_iSwapInitBlock.sub(fromBulkNumber.gt(0) ? fromBulkNumber.sub(1).mul(_commonDifference) : 0))
  ...
  if (currentPreviousBulkLastBlock.gt(lastRewardBulkLastBlock)) {
    ...
    if (N.gt(1)) {
      ...
      const diff = _bulkBlockSize.mul(_iSwapInitBlock.sub(currentBulkNumber.sub(2).mul(_commonDifference)))
      totalReward = totalReward.add(diff)
      console.log('diff ' + diff)
    }
  }
...
describe('iSwapMaster', () => {
  it('getReward', () => {
    ...
    let iSwapInitBlock = iSwapStartBlock
    let bulkBlockSize = bonusBeforeBulkBlockSize
    let commonDifference = bonusBeforeCommonDifference
    console.log('--------------------------')
    console.log('from ' + from)
    console.log('to ' + to)
    console.log('iSwapInitBlock ' + iSwapInitBlock)
    console.log('bulkBlockSize ' + bulkBlockSize)
    console.log('commonDifference ' + commonDifference)
    let totalReward = getTotalRewardInfoInSameCommonDifference(from, to, iSwapInitBlock, bulkBlockSize, commonDifference)

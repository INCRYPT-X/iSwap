...
import '@iSwapProject/iSwap-swap-lib/contracts/math/SafeMath.sol';
import '@iSwapProject/iSwap-swap-lib/contracts/token/BEP20/IBEP20.sol';
import '@iSwapProject/iSwap-swap-lib/contracts/token/BEP20/SafeBEP20.sol';
import '@iSwapProject/iSwap-swap-lib/contracts/access/Ownable.sol';

import './iSwapToken.sol';

contract iSwapMaster is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (userInfo.amount * pool.acciSwapPerShare) - userInfo.rewardDebt
        //
    ...
    // iSwap tokens created first block.
    uint256 public iSwapStartBlock;
    ...
    // iSwap tokens created at bonus end block.
    uint256 public iSwapBonusEndBlock;
    // max reward block
    ...
    // Accumulated iSwaps per share, times 1e12.
    uint256 public acciSwapPerShareMultiple = 1E12;
    // The iSwap TOKEN!
    ...
    constructor(
        ...
        iSwap = _iSwap;
        devAddr = _devAddr;
        iSwapStartBlock = _iSwapStartBlock;
        ...
        // iSwap created when bonus end first block.
        // (iSwapStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        iSwapBonusEndBlock = iSwapStartBlock
        ...
        // max mint block number, _iSwapInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_iSwapInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(iSwapBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_iSwapInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            totalReward = tempTo
                ...
                .mul(_iSwapInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _iSwapInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempiSwapInitBlock = _iSwapInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempiSwapInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    function pendingiSwap(address _pair, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 iSwapReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            acciSwapPerShare = acciSwapPerShare.add(iSwapReward.mul(acciSwapPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(acciSwapPerShare).div(acciSwapPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 iSwapReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        iSwap.mintTo(devAddr, iSwapReward.div(100));
        iSwap.mintTo(address(this), iSwapReward);
        pool.acciSwapPerShare = pool.acciSwapPerShare.add(iSwapReward.mul(acciSwapPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to iSwapMaster for iSwap allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.acciSwapPerShare).div(acciSwapPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
        ...
        userInfo.rewardDebt = userInfo.amount.mul(pool.acciSwapPerShare).div(acciSwapPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from iSwapMaster.
